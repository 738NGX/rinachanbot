<div align="center"><h1> koishi-plugin-rinachanbot</h1></div>
...
在下文的指令介绍中指令均采用以下格式:`/指令名 <必需参数> [可选参数] ... -选项1 <选项1参数> -选项2 ...`,之后不再赘述.部分指令暂时未继承到本项目中,会在
...
- `/倒数日`系列指令
    - `/倒数日 -a <倒数日名> -y <年份> -m <月份> -d <日期>`:在指定的日期添加一个倒数日.
        - `倒数日名`-接受一个字符串输入(不能有空格).
        - `-y <年份> -m <月份> -d <日期>`-接受一个合法的数字输入.输入非数字时抛出错误,缺省时默认采用当前时间的值,输入不合法的数字时会自动偏移修正.
        - 添加倒数日数量不允许超过后台规定的上限(默认为10).超过上限会报错.
        - **使用举例**-增加一个在今年10月6日的倒数日"亚巡上海场":`倒数日 -a 亚巡上海场 -m 10 -d 6`.

    - `/倒数日 -r <倒数日名>`:从列表中移除指定的倒数日.
    - `/倒数日 -l -y <年份> -m <月份> -d <日期>`:从指定的日期计算倒数日,不加任何参数(`/倒数日 -l`)代表从今日开始计算.
...
- `/新建账本 <账本名> -c <默认货币> -u <关联用户> -l <限额>`
- `/修改账本 <账本名> -c <默认货币> -u <关联用户> -l <限额>`
- `/合并账本 <账本名> <目标账本名>`
- `/删除账本 <账本名>`
- `/查账 [账本名] -r <参考汇率>`
- `/记账 <账本名> <项目名> <金额> -c <币种> -y <年份> -m <月份> -d <日期> -n <备注>`
- `/删账 <账目id>`
