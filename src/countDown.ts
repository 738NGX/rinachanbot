...
export async function listCountDown(day: any, month: any, year: any, group: string, ctx: Context) {
  ...
  const message = countDown.filter((cd: any) => cd.group == group).map((cd: any) => `距离${cd.name}${compare(date, cd.date) ? '还剩' : '已过'}${Math.abs(cd.date.getTime() - date.getTime()) / (24 * 3600 * 1000)}天`).join('\n');
  return message;
}
